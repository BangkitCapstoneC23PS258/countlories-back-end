...
router.route('/login').post(function(req, res) {
    ...
    if (tag == "login") {
        if (errors) {
            ...
            db.query('select * from user where email = ? and password = ?', [email, password], function(err, rows, fields) {
                if (err) throw err;
...
router.route('/register').post(function(req, res) {
    ...
    if (tag === "register") {
        ...
        db.query('select * from user where email = ?', [email], function(err, rows, result) {
            if (err) throw err;
            if(rows.length > 0){
                res.json({success: "0", message: "email exists"});
            }else {
                //check username exists
                db.query('select * from user where username = ?', [username], function(err, rows, result) {
                    if (err) throw err;
                    if(rows.length > 0){
                        res.json({success: "2", message: "username exists"});
                    }else {
                        db.query('INSERT INTO user VALUES(null, ?, ?, ?)', [username, email, password], function(err, result) {
                          if (err) throw err;
